...
module.exports = class Alphabet {
    ...
    clone() {
        ...
        copy.letterOrderIndexes = cloneObject(this.letterOrderIndexes);
    ...
    matchCase(referenceLetter, value) {
        var i, order, orderNames, refIndex, valueIndex;
        ...
        orderNames = Object.keys(this.letterOrder);

        for (i = 0; i < orderNames.length; i += 1) {
            order = this.letterOrder[orderNames[i]];
            refIndex = order.indexOf(referenceLetter);

            if (refIndex > -1) {
                return order.charAt(valueIndex);
            }
        }
    ...
    /**
     * Convert an index back to a letter. Invalid indexes are changed to
     * an empty string.
     *
     ...
     */
    toLetter(i) {
        if (i >= 0 && i < this.letterOrder.upper.length) {
