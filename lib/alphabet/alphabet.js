...
module.exports = class Alphabet {
    ...
    collapse(from, to) {
        ...
        if (fromIndex !== -1 && toIndex !== -1) {
            Object.keys(copy.characterSets).forEach((setName) => {
                var characters;

                characters = copy.characterSets[setName];

                // Set up the translation before the indexes change
                copy.translations[characters.charAt(fromIndex)] = characters.charAt(toIndex);

                // Remove the character
                characters = characters.substr(0, fromIndex) + characters.substr(fromIndex + 1);
                copy.characterSets[setName] = characters;
            });
        }
    ...
    isLetter(c) {
        ...
        for (i = 0; i < keys.length; i += 1) {
            if (this.characterSets[keys[i]].indexOf(c) !== -1) {
                return true;
            }
        }
