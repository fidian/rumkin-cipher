 ...
 * @property {boolean} homophonic True if one plaintext letter can map to
 *     multiple ciphertext letters. False if monophonic, where one plaintext
 *     letter can only map to one ciphertext letter.
 */
 ...
 * @property {WordDecipherList} wordDecipherList
 * @property {Word} word
 ...
 * @property {number} index The "active" word being tested in
 *     wordDecipherList.availableDeciphers
 */
...
module.exports = class Solver {
    ...
    callProgressCallback(statusCallback, state) {
        ...
        for (let i = 0; i < state.wordIndex; i += 1) {
            ...
            wordsBeingEvaluated.push({
                word: wordState.wordDecipherList.availableDeciphers[wordState.index],
    ...
    /**
     * Eliminate words from copy's availableDeciphers.
     *
     * @param {EliminationState} state
     */
    commitEliminationState(state) {
        for (const wordState of state.wordState) {
            wordState.wordDecipherList.availableDeciphers =
                wordState.wordDecipherList.availableDeciphers.filter((w) =>
     ...
     * wordlist words.
     *
     * @param {Array.<WordDecipherList|NonWord>} items
     * @param {StatusCallback} [options]
     * @param {StatusCallback} [statusCallback]
     * @return {Promise.<Array.<WordDecipherList|NonWord>>} Modified clone
     *     of items
     */
    eliminateBadCombinations(items, options, statusCallback) {
        ...
        const wordDeciphers = copy.filter((item) => item instanceof WordDecipherList);
        wordDeciphers.sort((a, b) => this.sortWordDecipherListsForSolving(a, b));

        const state = {
            options,
            wordState: wordDeciphers.map((wordDecipherList) => {
                return {
                    wordDecipherList,
                    word: wordDecipherList.word,
                    length: wordDecipherList.availableDeciphers.length,
        ...
        if (nextWordState) {
            ...
            for (const wordState of state.wordState) {
                wordState.validSolutions.add(
                    wordState.wordDecipherList.availableDeciphers[wordState.index]
                );
            }
    ...
    /**
     * Sort function comparing two words within WordDecipherLists to see which
     * one should be tackled first.
     *
     * @param {WordDecipherList} a
     * @param {WordDecipherList} b
     * @return {number}
     */
    sortWordDecipherListsForSolving(a, b) {
        // Sort first by the words with the most unique
        // characters
        const distinctCount = b.word.pattern.distinctCount - a.word.pattern.distinctCount;

        if (distinctCount) {
            return distinctCount;
        }

        // Next, try to pick one that has fewer available decipher options
        const availableDeciphers =
            a.availableDeciphers.length - b.availableDeciphers.length;

        if (availableDeciphers) {
            return availableDeciphers;
        }

        // Finally, use the longer word
        return b.word.chars.length - a.word.chars.length;
    }
    ...
    updateWordStateFromPrevious(options, prevWordState, nextWordState) {
        ...
        const word = prevWordState.wordDecipherList.availableDeciphers[prevWordState.index];
    ...
    wordWorksForMap(options, wordState) {
        ...
        const word = wordState.wordDecipherList.availableDeciphers[wordState.index];
